...
instance Nat.instLinearOrder : LinearOrder Nat where
  ...
  -- Strict order characterization: a < b ↔ (a ≤ b) ∧ ¬(b ≤ a)
  ...
  -- Totality: for any a and b, either a ≤ b or b ≤ a
  le_total a b := by
    obtain h | rfl | h := trichotomous a b
    . left; exact le_of_lt h    -- Case: a < b, so a ≤ b
    . simp [ge_refl]            -- Case: a = b, so a ≤ b (reflexivity)
    . right; exact le_of_lt h   -- Case: a > b, so b ≤ a

  -- Decidability: we can algorithmically decide whether a ≤ b
