...
classical
intro h
have hx : (empty : Object) ∈ singleton_empty := by
  simp [singleton_empty]
...
theorem SetTheory.Set.ssubset_trans (A B C:Set) (hAB:A ⊂ B) (hBC:B ⊂ C) : A ⊂ C := by
  ...
  constructor
  . exact SetTheory.Set.subset_trans hab1 hbc1
  . intro hAC
    have hCB : C ⊆ B := by simpa [hAC] using hab1
    have hBC : B = C := subset_antisymm B C hbc1 hCB
    contradiction
...
theorem SetTheory.Set.specify_subset {A:Set} (P: A → Prop) : A.specify P ⊆ A := by
  apply (SetTheory.specification_axiom A P).1
